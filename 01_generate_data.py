...
def generate_tascar_project(rotation: float = 0.0) -> str:
    ...
    pos = objectify.SubElement(source, "position")._setText(f"0 {sec_src_coords[0]} {sec_src_coords[1]} {sec_src_coords[2]}")
    
    # Add hrtf receiver with specified orientation
    rec = objectify.SubElement(scene, "receiver", name="hrtf_receiver",type="hrtf", nf_filter="true")
...
def render_tascar_project(rotation: float = 0.0):
    ...
    # Render project
    cmd = "tascar_renderfile --verbose --srate " + str(fs) + " --inputfile impulse.wav --outputfile " + fn + ".wav " + fn + ".tsc"
...
def main():
    ...
    os.mkdir("data/TASCAR_IRs") if not os.path.exists("data/TASCAR_HRTFs") else None
